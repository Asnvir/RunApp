...
class TrackingService : LifecycleService() {
    ...
    private fun updateLocationTracking(isTracking: Boolean) {
        if (isTracking) {
            if (TrackingUtility.hasLocationPermissions(this)) {
                val request =
                    LocationRequest.Builder(Constants.LOCATION_UPDATE_INTERVAL)
                        .setMinUpdateIntervalMillis(Constants.FASTEST_LOCATION_INTERVAL)
                        .setPriority(Constants.LOCATION_REQUEST_PRIORITY)
                        .build()
    ...
    private fun startForegroundService() {
        ...
        // observe the timeRunInMillis and update the notification
